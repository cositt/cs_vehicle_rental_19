...
class VehicleContract(models.Model):
    ...
    def a_draft_to_b_in_progress(self):
        ...
        for rec in self:
            ...
            # Check for overlapping contracts (in-progress) for the same vehicle
            ...
            if overlapping_contracts:
                # Construir mensaje con detalles de los contratos que se solapan
                conflicting_refs = ', '.join(overlapping_contracts.mapped('reference_no'))
                conflict_details = ""
                for contract in overlapping_contracts[:3]:  # Mostrar máximo 3
                    start = contract.start_date.strftime('%d/%m/%Y %H:%M')
    ...
    def _contract_check_dates(self):
        """Rental contract dates and overlapping validation"""
        for record in self:
            # Validación básica: fecha de fin debe ser mayor que fecha de inicio
            ...
            if record.status in ['b_in_progress', 'c_return', 'd_done'] and record.vehicle_id and record.start_date and record.end_date:
                ...
                overlapping = self.env['vehicle.contract'].search(domain, limit=1)
                
                if overlapping:
                    raise ValidationError(_(
                        f"No se puede procesar este contrato. Las fechas ({record.start_date.strftime('%d/%m/%Y')} - {record.end_date.strftime('%d/%m/%Y')}) "
                        f"se solapan con el contrato {overlapping.reference_no} "
                        f"({overlapping.start_date.strftime('%d/%m/%Y')} - {overlapping.end_date.strftime('%d/%m/%Y')}) "
                        f"para el mismo vehículo {record.vehicle_id.name}."
                    ))
    ...
    # -----------------------
    # Acciones: imprimir / enviar contrato
    # -----------------------
    def action_print_contract(self):
        """Imprimir PDF del contrato"""
        self.ensure_one()
        action = self.env.ref('vehicle_rental.vehicle_contract_report', raise_if_not_found=False)
        if not action:
            raise UserError(_("No se encontró el reporte de contrato."))
        return action.report_action(self)

    def action_send_contract_email(self):
        """Generar PDF y enviar por email usando la plantilla"""
        self.ensure_one()
        # Renderizar PDF usando API de nivel modelo (evita pasar lista como xmlid)
        report_xmlid = 'vehicle_rental.action_vehicle_contract_report'
        pdf_content, _dummy = self.env['ir.actions.report']._render_qweb_pdf(
            report_xmlid,
            [self.id]
        )
        filename = f"Contrato_{self.reference_no or self.id}.pdf"
        ...
        # Abrir el compositor de email con la plantilla y el PDF adjunto, sin enviar automáticamente
    ...
    def cancellation_charge_invoice(self):
        ...
        invoice_line = []
        for rec in self:
            # Permitir cancelaciones sin cargo (cancellation_charge = 0)
            if rec.cancellation_charge and rec.cancellation_charge > 0:
                cancellation_data = {
